...
import { UserIntent, VideoMetadata } from '@/types';
...
describe('RecommendationEngine', () => {
  ...
  describe('analyze method - website focus', () => {
    ...
    it('should warn about long videos for website purpose', () => {
      ...
      const videoMeta = createMockVideoMeta({ duration: 150 }); // 150 seconds (2.5 minutes)

      const result = engine.analyze(intent, videoMeta);

      expect(result.warnings).toContain('긴 영상은 변환 시간이 오래 걸릴 수 있습니다');
    });
  ...
  describe('size estimation', () => {
    it('should include estimated sizes for both formats', () => {
      ...
      expect(result.estimated_sizes.gif).toMatch(/\d+(\.\d+)?\s*MB/);
      expect(result.estimated_sizes.webm).toMatch(/\d+(\.\d+)?\s*MB/);
    });
  });
